...
# ╔═╡ f9a690ca-6e11-11ec-3533-b1b3935768d8
# prepare the tools
begin
	using DataFrames
	using Plots
...
# ╔═╡ 50b72b21-45bb-43d4-81e5-d35a6caa0e69
md"
...
# ╔═╡ 12ae1b09-8f49-42e4-9903-067cf788bce3
md"
## Sorting
...
# ╔═╡ 43271dc0-1999-4955-b004-c19319b3d6a7
# example
x⃗ = rand(1:10, 10)

# ╔═╡ 0a318d03-c17b-4a2c-af6b-498e8afeec5d
begin
	𝕹 = 10^4
	𝔵⃗ = rand(1:𝕹, 𝕹)
end

# ╔═╡ 8048740c-da11-4557-b511-ce0c7b88825d
md"
...
# ╔═╡ 8323eac9-5fa0-498a-b0f9-4b927d068ce9
...
# ╔═╡ ef40742e-afc9-49d6-b63b-91ef80f108d9
md"
...
# ╔═╡ 9dfc3454-5afa-49a3-8c96-b64643673c6b
δ(𝓍⃗ :: Vector{Int}, 𝓎⃗ :: Vector{Int}) = return sum(abs.(𝓍⃗ .- 𝓎⃗))

# ╔═╡ 619f3d1f-cd10-4c0e-9701-db952385fb0d
# example
(ϕ(x⃗), δ(x⃗, rand(1:10, 10))) # y⃗ = rand(1:10, 10)

# ╔═╡ e3436267-6a21-4b5e-b6eb-7e4450a0add0
md"
## Local Search
One need neighborhood operator in order to generate neighborhood solution if local search is needed to perform.
"

# ╔═╡ 977a9835-3d5c-4b60-80b4-b24311aac291
md"
...
# ╔═╡ 42337570-fa25-4790-8dc5-0d9c89bd686a
...
# ╔═╡ 40188566-01d2-45fd-9730-d58f749f9d19
...
# ╔═╡ d6811e66-812c-4e53-8242-cc46ed30da7f
md"The local search algorithm is computed as follows."

# ╔═╡ d88e4ef8-3453-41d1-aa73-e0e89cb2e76b
...
# ╔═╡ 7a7ac74e-f17a-41ee-b488-ab5ec974dfcf
# example
local_search(x⃗, 100)

# ╔═╡ a77a9f95-bbde-4896-b567-319642fae921
md"# Computation of Simulated Annealing"

# ╔═╡ 170a70a5-9187-4e00-a6d1-af89782aaca1
md"
...
# ╔═╡ 89bf371b-f752-4cf2-a8ad-5fdf50601f2b
function simulated_annealing_sort(𝒮 :: Vector{Int}, 𝓉₀ :: Float64, α :: Float64, max_iteration :: Int)
	𝓉_curr = 𝓉₀
	𝓉_lim = 𝓉_curr / max_iteration
	𝓋⃗ = [] # gathering ϕ values
	δ⃗ = [] # gathering δ values
	ℬ = [] # gathering the best value
	𝒹 = rand(0:0, length(𝒮)) # 𝒹 = (0, 0, …, 0)ᵀ
	curr_sol = copy(𝒮)
	curr_obj = ϕ(curr_sol)
	push!(𝓋⃗, curr_obj)
	push!(δ⃗, δ(𝒹, curr_sol))
	push!(ℬ, copy(curr_sol))
	𝒹 = copy(curr_sol)
...
# ╔═╡ 9b3bb270-543c-4707-a627-2b0194863622
# example
begin
	𝓉_init = 100.0
	alpha_ = 0.849
	max_iter = 10^3
	served_solution = simulated_annealing_sort(𝔵⃗, 𝓉_init, alpha_, max_iter)
end

# ╔═╡ 03431c29-0c15-4392-ac08-fab7abc8b68c
begin
	curr_df = served_solution[2]
	describe(curr_df)
end

# ╔═╡ 1d1d76b4-58f7-48e3-9a7b-3ef6b3f81fb5
# proportion of ϕ₀ : ϕₙ
curr_df[1,2] / curr_df[length(curr_df[:,1]), 2]

# ╔═╡ 8425a0bd-280e-4c47-8a94-f4d224b549dd
# filter
[x for x in served_solution[1] if ϕ(x) ≥ ϕ(sort(𝔵⃗))]

# ╔═╡ 15263c6a-659c-4e9c-8c63-5679d375a0a6
md"
# Visualization
"

# ╔═╡ a4693719-29a0-4f4b-934e-0fc31f9820ae
plot(
	plot(curr_df[:,1], curr_df[:,2], 
		title = "SA Sorting for #x⃗ = 10^4", label = "ϕ-val",
		ylab = "ϕ(x⃗)", legend = :bottomright, theme(:ggplot2)),
	plot(curr_df[:,1], curr_df[:,3], 
		title = "SA Sorting for #x⃗ = 10^4", label = "δ-val",
		ylab = "δ(x⃗, y⃗)", theme(:ggplot2)), layout = (2,1))

# ╔═╡ 0ebd76b5-5de0-49c8-b6c1-2277dc65cbcf
ϕ(sort(𝔵⃗))

# ╔═╡ 60a015e2-0538-4eed-98db-6f61eb37f882
